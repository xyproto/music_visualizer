...
ShaderPrograms::ShaderPrograms(const ShaderConfig& config, const Renderer& renderer,
    const Window& window, const filesys::path& shader_folder)
{
#define lambda [&](const int p, const Buffer& b)
    builtin_uniforms = { { "vec2", "iMouse",
        lambda { glUniform2f(get_uniform_loc(p, 0), window.mouse.x, window.mouse.y);
}
}
...
,
    { "vec2", "iMouseDownPos",
        lambda {
            glUniform2f(get_uniform_loc(p, 2), window.mouse.last_down_x, window.mouse.last_down_y);
}
}
...
for (const Buffer& b : config.mBuffers)
    compile_buffer_shaders(
        shader_folder, b.name, uniform_header.str(), b.uses_default_geometry_shader);
...
void ShaderPrograms::compile_buffer_shaders(const filesys::path& shader_folder,
    const string& buff_name, const string& uniform_header, const bool uses_default_geometry_shader)
{
    ...
    if (frag_str.str().find("mainImage", uniform_header.size() + version_header.size())
        != std::string::npos)
