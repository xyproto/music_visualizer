...
static Buffer parse_image_buffer(rj::Document& user_conf)
{
    ...
    if (image.HasMember("clear_color")) {
        ...
        if (!(clear_color.IsArray() && clear_color.Size() == 3))
            throw runtime_error(
                "image.clear_color must be an array of 3 real numbers each between 0 and 1");
        for (int i = 0; i < 3; ++i) {
...
static Buffer parse_buffer(rj::Value& buffer, string buffer_name, set<string>& buffer_names)
{
    ...
    if (buffer_names.count(b.name))
        throw runtime_error(
            "Buffer name " + b.name + " already used (buffers must have unique names)");
    buffer_names.insert(b.name);
    if (!(std::isalpha(b.name[0]) || b.name[0] == '_'))
        throw runtime_error("Invalid buffer name: " + b.name
    ...
    if (buffer.HasMember("clear_color")) {
        ...
        if (!b_clear_color.IsArray() || b_clear_color.Size() != 3) {
            throw runtime_error(
                b.name + ".clear_color must be an array of 3 real numbers each between 0 and 1");
        }
        for (int i = 0; i < 3; ++i) {
            if (b_clear_color[i].IsNumber())
                ...
                throw runtime_error(b.name
                    + ".clear_color must be an array of 3 real numbers each between 0 and 1");
    ...
    if (!buffer.HasMember("size")) {
        cout << "Buffer: " << b.name
             << " does not have a size option, assuming the size of the buffer to be the window "
                "size"
             << endl;
    } else {
    ...
    if (!buffer.HasMember("geom_iters")) {
        cout << "Buffer: " << b.name
             << " does not have a geom_iters option, assuming the number of times to execute "
...
static vector<int> parse_render_order(rj::Document& user_conf, vector<Buffer>& buffers)
{
    ...
    if (!render_order.IsArray() || render_order.Size() == 0)
        throw runtime_error(
            "render_order must be an array of strings (buffer names) with length > 0");
...
Uniform parse_uniform(rj::Value& uniform, string uniform_name, set<string>& uniform_names)
{
    ...
    if (uniform_names.count(uniform_name))
        throw runtime_error(
            "Uniform name " + uniform_name + " already used (uniforms must have unique names)");
    ...
    if (uniform.IsArray()) {
        if (uniform.Size() > 4)
            throw runtime_error(
                "Uniform " + u.name + " must have dimension less than or equal to 4");
        ...
        throw runtime_error(
            "Uniform " + u.name + " must be either a number or an array of numbers.");
    }
    return u;
}
...
void ShaderConfig::parse_config_from_string(const std::string& json_str)
{
    ...
    if (user_conf.HasMember("initial_window_size")) {
        ...
        if (!(window_size.IsArray() && window_size.Size() == 2 && window_size[0].IsInt()
                && window_size[1].IsInt()))
...
ShaderConfig::ShaderConfig(const filesys::path& shader_folder, const filesys::path& conf_file_path)
{
    ...
    if (filesys::exists(filesys::path(shader_folder / (mImage.name + ".geom")))) {
        ...
        cout << "Warning: Buffer " << mImage.name
             << " is using default geometry shader and has geom_iters > 1 set. Performance could "
                "suffer."
             << endl;
    }
    for (Buffer& b : mBuffers) {
        if (filesys::exists(filesys::path(shader_folder / (b.name + ".geom")))) {
            ...
            cout << "Warning: Buffer " << b.name
                 << " is using default geometry shader and has geom_iters > 1 set. Performance "
                    "could suffer."
                 << endl;
        }
    }
...
ShaderConfig::ShaderConfig(const string& json_str) { parse_config_from_string(json_str); }
