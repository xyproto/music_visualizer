...
LinuxAudioStream::LinuxAudioStream()
{
    ...
    pulseState = pa_simple_new(NULL, "Music Visualizer", PA_STREAM_RECORD, sink_name.c_str(),
        "Music Visualizer", &pulseSampleSpec, NULL, &pb, &pulseError);
    if (!pulseState) {
        cout << "Could not open pulseaudio source: " << sink_name.c_str() << " "
             << pa_strerror(pulseError)
...
void LinuxAudioStream::get_next_pcm(float* buff_l, float* buff_r, int size)
{
    ...
    if (pa_simple_read(pulseState, buf_interlaced, size * channels * sizeof(float), &pulseError)
        < 0) {
